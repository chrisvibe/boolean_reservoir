...
class InputParams(BaseModel):  # TODO split into Bits layer (B→I) and Input layer (I→R)
    ...
    w_ir: Union[str, List[str]] = Field(
        'out-1:1:1', description="See w_bi property. Produces a biparitite mapping from input nodes to reservoir nodes (I→R) so the following symbolic tokens are replaced accordingly; 'a': I, 'b': R")
    w_in: Optional[Union[Path, List[Path]]] = Field(
        None, description="Input distribution mapping explicitely set by an adjacency matrix B->I. Parameter is a path to a stored tensor. Overrides distribution parameter")
    selector: Union[str, List[str]] = Field(
        'S :I', description="Selection chain. Supports F (filter), S (slice), R (random). Variables: i: index variable for F operation, I: I.n_nodes. Ie. assuming I=2 F i<4 -> S -3: -> R I ([0, 1, 2, 3]->[1, 2, 3]->[1, 3]). R samples: R without samples => scramble")
    pertubation: Union[str, List[str]] = Field(
        'xor', description="Pertubation strategy given old and new states for input nodes")
    encoding: Union[str, List[str]] = Field(
    ...
    features: Union[int, List[int]] = Field(
        None, description="Dimension of input data before binary encoding")
    bits: Optional[Union[int, List[int]]] = Field(
        None, description="Total bits after encoding")
...
class ReservoirParams(BaseModel):
    ...
    k_min: Union[int, List[int]] = Field(
        0, description="Min degree of incoming nodes")
    k_avg: Union[float, List[float]] = Field(
        2.0, description="Average degree of incoming nodes")
    k_max: Optional[Union[str, List[str], int, List[int]]] = Field(
        None, description="Maximum degree of incoming nodes")
    mode: Union[str, List[str]] = Field(
        'heterogeneous', description="heterogeneous: each node can have different number of neighbours, homogeneous: each node has k neighbours set by k_avg")
    ...
    reset: Optional[Union[bool, List[bool]]] = Field(
        True, description="Reset to init state after each sample")
    self_loops: Optional[Union[float, List[float]]] = Field(
        None, description="Probability of self-loops in graph; normalized by number of nodes")
    init: Union[str, List[str]] = Field(
        'random', description="Initalization strategy for reservoir node states")
...
class OutputParams(BaseModel):
    seed: Optional[int] = Field(
        None, description="Random seed, None disables seed")
    n_outputs: Union[int, List[int]] = Field(
        1, description="Dimension of output data")
    activation: Optional[Union[str, List[str]]] = Field(
        None, description="Activation after readout layer, fex sigmoid")
    readout_mode: Optional[Union[str, List[str]]] = Field(
