...
def load_data(variable):
    ...
    df_metric = df_metric[df_metric['D_tao'] == 3] # metric for prediction
...
def process_grid_search_data(df, d_set, i_set, r_set):
    ...
    if df.iloc[0]['params'].L.train_log.loss:
        df['loss'] = df['params'].apply(lambda p: p.L.train_log.loss ** .5) # MSE to RMS
...
def anova_analysis(out_path: Path):
    ...
    for name, (h_stat, p_value) in normality_results.items():
        print(f"{'-'.join(map(str, name))}: h-statistic={h_stat:.3f}, p-value={p_value:.3f}")

    ######## HOMOGENEITY ###########################################
    homogeneity_h_stat, homogeneity_p_value = check_homogeneity(groups_dict)
    print(f"\nHomogeneity of Variances (Levene's Test):")
    print(f"h-statistic={homogeneity_h_stat:.3f}, p-value={homogeneity_p_value:.3f}")
